// REVE CRM · Netlify Function — integrada com dados do app
...
if(op==="patients_list"){
  await assertAdmin(sb,auth);
  const q=body.q||"";
  let query=sb.from("crm_patient_summary").select("*").order("full_name");
...
if(op==="patient_upsert"){
  await assertAdmin(sb,auth);
  const{user_id,full_name,email,phone,city,birth_date}=body;
  if(!user_id||!full_name)return err("user_id e full_name obrigatórios");
  const{error}=await sb.from("crm_patients").upsert({user_id,full_name,email,phone,city,birth_date:birth_date||null,updated_at:new Date().toISOString()},{onConflict:"user_id"});
  if(error)throw error;
  return json({ok:true});
}
...
if(op==="plan_get_active"){
  ...
  const{data,error}=await sb.from("crm_plans").select("*").eq("user_id",user_id).eq("is_active",true).order("created_at",{ascending:false}).limit(1).maybeSingle();
  if(error)throw error;
...
if(op==="plan_save_active"){
  ...
  if(!user_id)return err("user_id obrigatório");
  await sb.from("crm_plans").update({is_active:false}).eq("user_id",user_id);
  const{error}=await sb.from("crm_plans").insert({user_id,title:title||"Plano",content:content||{},is_active:true,created_at:new Date().toISOString(),updated_at:new Date().toISOString()});
  if(error)throw error;
  return json({ok:true});
}
...
if(op==="diary_get"){
  ...
  const[cl,notes]=await Promise.all([
    sb.from("checklist").select("*").eq("user_id",user_id).order("key"),
    sb.from("notes").select("*").eq("user_id",user_id).order("note_id")
  ]);
  if(cl.error)throw cl.error;
  if(notes.error)throw notes.error;
  return json({checklist:cl.data,notes:notes.data});
}
...
if(op==="photos_get"){
  ...
  const{user_id}=body;
  if(!user_id)return err("user_id obrigatório");
  const{data,error}=await sb.from("photos").select("*").eq("user_id",user_id).order("updated_at",{ascending:false});
  if(error)throw error;
  return json({photos:data});
}

if(op==="patient_app_data"){
  await assertAdmin(sb,auth);
  const{user_id}=body;
  if(!user_id)return err("user_id obrigatório");
  const[summary,cl,notes,photos,plan]=await Promise.all([
    sb.from("crm_patient_summary").select("*").eq("user_id",user_id).maybeSingle(),
    sb.from("checklist").select("*").eq("user_id",user_id).order("key"),
    sb.from("notes").select("*").eq("user_id",user_id).order("note_id"),
    sb.from("photos").select("*").eq("user_id",user_id).order("updated_at",{ascending:false}),
    sb.from("crm_plans").select("*").eq("user_id",user_id).eq("is_active",true).limit(1).maybeSingle()
  ]);
  return json({
